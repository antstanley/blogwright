import type { SeoConfig } from '@iamstan/ops-core';
...
describe('resolveSeo', () => {
  it('production auto: indexable robots + sitemap referencing the origin', () => {
    const r = resolveSeo(ctx('production', false, AUTO), 'https://iamstan.dev');
    expect(r.robots).toContain('Allow: /');
    expect(r.robots).not.toContain('Disallow: /');
    expect(r.robots).toContain('Sitemap: https://iamstan.dev/sitemap.xml');
    expect(r.sitemapBaseUrl).toBe('https://iamstan.dev');
  });
  ...
  it('preview auto: treated as non-production (blocked, no sitemap)', () => {
    const r = resolveSeo(ctx('preview', true, AUTO), 'https://pr-2.preview.iamstan.dev');
    expect(r.robots).toContain('Disallow: /');
    expect(r.sitemapBaseUrl).toBeUndefined();
  });
