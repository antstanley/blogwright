...
describe('clientMetadata', () => {
  it('derives every URL from the domain', () => {
    expect(clientMetadataUrl('iamstan.dev')).toBe('https://iamstan.dev/oauth/client-metadata.json');
    expect(jwksUrl('iamstan.dev')).toBe('https://iamstan.dev/oauth/jwks.json');
    expect(callbackUrl('iamstan.dev')).toBe('https://iamstan.dev/oauth/callback');
  });

  it('builds a confidential web-client document (client_id = metadata URL)', () => {
    const meta = clientMetadata('iamstan.dev', PDS);
    expect(meta).toEqual({
      ...
      response_types: ['code'],
      redirect_uris: ['https://iamstan.dev/oauth/callback'],
