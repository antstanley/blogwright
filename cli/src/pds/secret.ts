...
import type { SecretsManagerClient } from '@iamstan/ops-core';
...
const SECRET_DESCRIPTION =
  'AT Protocol OAuth client key + session for standard.site publishing (blog-ops pds)';
  ...
  if (isLegacySecret(parsed)) {
    throw new Error(
      `secret "${secretName}" holds app-password credentials — app passwords are no longer ` +
        'supported; run `blog-ops pds keygen` then `blog-ops pds login`',
    );
  }
  if (parsed.version !== 1) {
...
export async function loadPdsSecret(ctx: OpsContext): Promise<PdsSecret> {
  ...
  if (!raw) {
    throw new Error(`no secret at "${pds.secretName}" — create it with \`blog-ops pds keygen\``);
  }
