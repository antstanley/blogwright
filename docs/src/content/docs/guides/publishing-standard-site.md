...
## Deploy-role access for CI

PDS is bundled with the CLI but owns a separate IAM policy. For every non-preview
stack with `githubRepo`, first create the site role with `blogwright bootstrap <env>`,
then run `blogwright pds bootstrap <env>` once to attach its secret-access grant.
When upgrading from the additive migration release, run PDS bootstrap before the
next site bootstrap removes the old grant. Staging is eligible; preview never
receives access to the shared PDS credential. If the step was missed, production
site deploys still succeed while PDS sync warns; PDS bootstrap repairs access.

`pds status <env>` reports the plugin node. `pds destroy <env> --yes` removes its
policy and scoped state, leaving the secret and site role. Site teardown refuses
while `state/<env>.pds.json` exists, and site bootstrap reminds recorded plugins
to reconcile their own resources. See [PDS lifecycle](/reference/cli/#pds-bootstrap-status-and-destroy).
