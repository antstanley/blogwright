...
Blogwright looks to fix that for a single use case. How do I publish a static webiste (HTML, CSS and JavaScript) to AWS? Whether that site is a static site, like your personal blog, or a single page app. It doesn't require you to learn how to use AWS in any way. All you need to do is login, and blogwright does the rest.

It bakes in best practices, and gives you advanced functonality like previews, and rollbacks. blogwright gives you full operations for a blog or static site on AWS. S3 + CloudFront hosting with the origin kept private via Origin Access Control, and the site built inside a **Lambda MicroVM**. One CLI - `blogwright`, alias `bw` - creates the infrastructure, runs deploys, serves PR previews, and can mirror your posts to the AT Protocol as [standard.site](https://standard.site) records.
