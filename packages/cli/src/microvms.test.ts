...
describe('runningStackMicrovms', () => {
  it('keeps only active VMs launched from this stack image', async () => {
    const ctx = fakeCtx([
      ...
      vm('c', 'RUNNING', 'arn:aws:lambda:us-east-1:1:microvm-image:production-example-builder'), // ✗ other env
  ...
  it('falls back to matching the image name when state has no ARN yet', async () => {
    ...
    // eslint-disable-next-line e18e/no-delete-property -- the fixture must not have the key at all.
