...
**Implements:** [2026-07-26-cli_plugin_system.md §State → Scoped state stores (Modify)](../../../changes/merged/2026-07-26-cli_plugin_system.md) (an unscoped store keys `state/<env>.json` as today; a store scoped to a plugin keys `state/<env>.<plugin>.json`)
**Depends on:** -
...
- [ ] Extend the class doc comment (`:21-24`) to state both key forms, why the unscoped one is on-disk identity that must not move for an existing environment, and that a scope changes the key alone: both objects live in the site's bucket under the same `state/` prefix, which `bucketNode.delete()` empties wholesale (`packages/cli/src/nodes.ts:66`). Name task 16's `blogwright destroy` guard as what keeps a site teardown from taking a plugin's record with it, so the next reader of this class does not mistake a scoped key for an isolated store.
