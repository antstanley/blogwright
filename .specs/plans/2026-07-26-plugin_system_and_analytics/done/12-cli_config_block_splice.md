...
- [ ] Reviewable: run `pnpm --filter blogwright exec vitest run config-block --reporter=verbose`; confirm the comment-preservation test asserts byte equality outside the insert, that every one of the five refusal tests asserts on the error message, and that no successful case is accepted without a `parseConfig` re-parse.
