> Historical planning/execution record through beta.3. Current scheduling and closure obligations are in [plan.md](plan.md); do not execute the superseded closure instructions below.
...
- **The first production run found two observability gaps and neither was a
  ...
  **The transform role granted `logs:CreateLogStream` and `logs:PutLogEvents`
  ...
  have got a group that appears at some unpredictable first invocation, retained
  forever, that nothing reconciles and nothing removes. Planned in
  ...
  **Firehose's own `CloudWatchLoggingOptions` is `Enabled: false`.** That is a
  larger fix and was deliberately kept separate from the gap above: Firehose
  does not create its log group, so enabling it means owning that group as a
  ...
  [`plans/2026-08-31-analytics_owned_log_groups/plan.md`](../2026-08-31-analytics_owned_log_groups/plan.md).
  Recorded here rather than left in a PR description, because this build's own
  evidence is that a finding no definition of done names is a finding that gets
  lost.
