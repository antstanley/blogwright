# Done Certificate — Task 53: Assemble the ten-node graph and run it through the plugin lifecycle verbs

**Task:** [53-analytics_graph_and_lifecycle_commands.md](53-analytics_graph_and_lifecycle_commands.md) · **Plan:** [plan.md](../plan.md)
...
- **O3 — The destroy refusal.**
  ...
  - *Evidence to collect:* run `pnpm test -- plugin-commands` in `packages/cli` and read the refusal case for a ten-node plugin — confirm it asserts the message text and an empty recorded call list; run `pnpm test -- plugin` in `packages/analytics` and confirm the table test asserts no `destroy` entry.
  - *Status:* ☐ unverified
...
- **O6 — Run both command suites and confirm the ten ids, the single state key and the region line (Reviewable).**
  - *Claim:* a reviewer can run the analytics and CLI command suites and observe an ordering test naming all ten ids, a recording S3 client showing only `state/<env>.analytics.json` for a plugin bootstrap, and captured bootstrap lines containing `us-east-1`.
...
- `packages/cli/src/commands.ts:54` `destroy(ctx, { yes: true })` with the analytics plugin installed → expect the plugin's ten resources untouched and `state/<env>.analytics.json` not deleted : ☐ (PRESERVED / REGRESSION)
- `packages/analytics/src/plugin.ts` `nodes(ctx)` called by task 16's generic verbs → expect the same ten-node array `buildAnalyticsNodes` returns, with no wrapper re-ordering it : ☐ (PRESERVED / REGRESSION)

## Residue

The engine-level assertions live in `packages/cli/src/plugin-commands.test.ts` because the analytics package may not depend on `blogwright`; the validator should confirm that split is real — a graph-shape test on the plugin side and an engine test on the CLI side — rather than an engine quietly reimplemented in the package. A site teardown leaves the plugin's delivery plumbing orphaned (the failure mode `packages/cli/src/nodes.ts:764` documents and both change specs raise as an open question); nothing in this DoD closes it. If task 16 decided a precedence other than the recommended one, the source of the bootstrap region line and the owner of the destroy refusal move with it.
