...
**Depends on:** 39
**Produces:** `mapRecord` in `packages/analytics/transform/map-record.ts` — a pure function turning one CloudFront access-log record into a `page_views` row or a droppable result naming the missing field, with `event_time` and `day` derived in UTC and covered by day-rollover boundary tests
**Pointers:** `packages/analytics/transform/map-record.ts` (new — the pure mapper), `packages/analytics/transform/map-record.test.ts` (new — fixed-input/fixed-output and boundary tests), `packages/analytics/src/schema.ts` (task 39 — `PAGE_VIEWS_COLUMNS`, `FIELD_TO_COLUMN`, `DERIVED_COLUMNS`, the `PageView` row type this consumes), `packages/analytics/vitest.config.ts:8` (the `include` glob, which must widen from `src/**/*.test.ts` to cover `transform/**/*.test.ts`), `packages/analytics/package.json:19-24` (the `lint` script `oxlint src`, which must widen to cover `transform`), `packages/build-agent/src/build.ts` (the precedent for a bundled, non-`src` module tree in this workspace)

## Steps

- [ ] Widen `packages/analytics/vitest.config.ts` `include` to `['src/**/*.test.ts', 'transform/**/*.test.ts']` and the package `lint` script to `oxlint src transform`, so the transform tree is linted and tested by the same gates as `src`.
- [ ] Write `mapRecord(record)` in `packages/analytics/transform/map-record.ts` returning a discriminated result — a mapped `PageView` row, or a droppable outcome carrying the reason and the missing field name — never a partially populated row and never `null`.
...
- [ ] Write `packages/analytics/transform/map-record.test.ts` with fixed record fixtures and fully spelled expected rows, plus a drop case per required column and the two day-rollover boundary cases at `23:59:59.999` and `00:00:00.000` UTC.
