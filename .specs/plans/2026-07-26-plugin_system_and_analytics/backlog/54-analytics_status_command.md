...
**Depends on:** 45, 53
**Produces:** `analytics status` lists the ten nodes present or missing against `state/<env>.analytics.json` in the same pretty/plain split as the site's `status`, and appends the Firehose stream's delivery health and the table's row count, degrading each to a warning when its read fails
...
- [ ] Reviewable: run `pnpm test -- commands` inside `packages/analytics`; confirm the plain-mode test asserts ten lines by node title, that the two degraded cases still show all ten, and that the never-bootstrapped case asserts a zero exit rather than only the absence of a throw.
