...
- [ ] The salt decision is SETTLED (2026-07-26): a random secret held in Secrets Manager, rotated daily — never derived from the date. Record it as a decision note in the module doc comment with the reason: IPv4 is a 2^32 space, so a salt an attacker can compute makes the digest brute-forceable in seconds and the pseudonymisation decorative. Also note that changing the salt after rows exist breaks unique-visitor comparison across the boundary.
- [ ] Take the salt as an ARGUMENT — `visitorKey(ip, userAgent, salt)` — so this module stays pure and testable with a pinned vector, and no part of it reads Secrets Manager. Fetching and caching the salt is task 42's envelope concern (cold-start read, cached for the execution environment's life); the node that provisions the secret and the grant is task 50's.
- [ ] Write `visitorKey(ip, userAgent, salt)` in `packages/analytics/src/transform/visitor-key.ts` as a SHA-256 digest over the three inputs with an unambiguous separator, returning a hex string; the salt is a parameter derived from an injected day, never read from a wall clock inside the function.
...
- [ ] Write the bot matcher in `packages/analytics/src/transform/bots.ts` over a named module constant of user-agent patterns, returning a boolean for every input including an absent or empty user agent.
