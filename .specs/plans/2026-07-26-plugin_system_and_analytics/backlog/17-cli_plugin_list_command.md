...
- [ ] Reviewable: run `pnpm --filter blogwright exec vitest run plugin-commands --reporter=verbose`; confirm the mixed good/broken listing shows the healthy plugin's version sourced from the in-memory `package.json` the test wrote, and that no test in the file constructs a real `ModuleLoader` or reads from disk.
