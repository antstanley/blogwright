...
- [ ] Add `resolvePluginConfig(plugin, config)` in `plugins.ts` taking ONE plugin: when it declares both `configKey` and `validateConfig`, read the raw block with `pluginBlock(config, configKey)` (task 03) and pass it to the validator, RETURNING the resolved block for the dispatcher to place on `ctx.pluginConfig`. Skip when the key is absent — never pass `undefined` into a plugin, per DEVELOPMENT.md's no-null rule.
- [ ] Call it from `runPlugin` (task 10) immediately after the context is built and before the plugin's `run`, assigning the result to `ctx.pluginConfig`, so an invalid block fails before the command does any work and a valid one is typed for the plugin. Built-in commands never reach it.
