...
- [ ] If eager: route the parsed config through task 19's plugin validation from `createContext` (`packages/cli/src/context.ts:120`), and bound the cost — a repo with no plugin installed must not pay a full plugin load on `deploy`/`status`, proven by a test that counts `ModuleLoader` calls.
