...
DONE(Task 27) ≡ every obligation O1…O6 below holds, each backed by the evidence the obligation
...
- **O4 — Every `secretName` reader compiles without a non-null assertion, and a changeset states the impact.**
  - *Claim:* `packages/pds` (via task 22) and `packages/cli/src/nodes.ts:925` (via task 23) compile against the now-optional field with no `!` and no cast, and a changeset records the semver impact of removed validation plus an optional published field.
  - *Evidence to collect:* run `pnpm typecheck` from the repo root and expect clean; run `grep -rnE "secretName!|pds!" packages packages --include=*.ts` and expect no hits; read the new `.changeset/*.md` and confirm it names `blogwright-core` with a bump and states both consequences.
  - *Checks:* resolve `resolvePdsSecretName` at `packages/cli/src/nodes.ts:925` — confirm it is the `blogwright-pds` export (task 23), because with core's default gone this call is now the only thing standing between the IAM policy and `secret:undefined-*`.
  - *Status:* ☐ unverified
...
- **O6 — Reviewable: gates plus a grep of core's config module (Reviewable).**
...
core-drops-pds change; discharging it discharges both. Ordering is load-bearing in two directions:
it must not land before task 23, or the OIDC deploy role silently degrades to `secret:undefined-*`
(a wrong permission grant, not a crash); and it must ship in the same release as task 28, because
the validation-timing gap it opens only becomes observable once core stops validating. A validator
should confirm both orderings held, not merely that the tests pass. Not covered by the DoD: config
