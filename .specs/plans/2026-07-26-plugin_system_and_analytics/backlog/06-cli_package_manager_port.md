...
- [ ] Reviewable: run `pnpm --filter blogwright test -- process-package-manager` and `grep -rn "node:child_process" packages/cli/src`; confirm every `detect` case passes against an in-memory filesystem with no process spawned, and that the grep returns only files under `packages/cli/src/adapters/`.
