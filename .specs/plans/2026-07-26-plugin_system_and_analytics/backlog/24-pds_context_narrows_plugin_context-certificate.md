...
DONE(Task 24) ≡ every obligation O1…O6 below holds, each backed by the evidence the obligation
...
- **O4 — The pds test factory stays cheap.**
...
- **O6 — Reviewable: `pnpm typecheck && pnpm test` with an unchanged test count plus one (Reviewable).**
...
deleted. Verify O3 by the deliberate-break procedure, not by its presence. Not covered by the DoD:
whether `PluginContext` carries a scoped `StateStore` field that pds does not use (task 01 decides);
if it does, the narrowing must exclude it so `packages/pds/src/test-support.ts` need not build one.
