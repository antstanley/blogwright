...
- [ ] Add a private `call<T>(method, path, payload?)` helper in the `packages/core/src/aws/microvms.ts:124-140` shape, sending the `lambda` service descriptor through task 31's seam with `content-type: application/json`.
