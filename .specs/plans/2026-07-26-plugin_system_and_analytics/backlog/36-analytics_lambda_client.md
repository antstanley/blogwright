# Task 37 — LambdaClient in blogwright-core, distinct from MicrovmsClient
...
- [ ] Write `packages/analytics/src/aws/lambda.test.ts` with the `packages/core/src/aws/logs.test.ts:9-16` transport helpers: one recording case per operation pinning method, path and body, an assertion over every recorded URL that none contains `/2025-09-09/`, an absent-function `getFunction` returning `undefined`, a not-found `deleteFunction` resolving, and a `500` rejecting.
