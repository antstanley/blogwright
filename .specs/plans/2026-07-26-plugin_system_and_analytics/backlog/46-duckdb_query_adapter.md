...
- [ ] Reviewable: run `pnpm test -- duckdb-query` inside `packages/analytics`; confirm the credential test asserts the literal `staticCredentials` access key and session token reaching the secret statement, and that `grep -rn "@duckdb" packages/analytics/src/` matches only `adapters/duckdb-query.ts`.
