...
- **O6 — The pinned keys and the unchanged call sites are visible in two commands (Reviewable).**
...
- `packages/cli/src/commands.ts:63` calls `ctx.store.delete()` during `destroy` → expect a DELETE of `state/<env>.json`, still swallowing its own failure : ☐ (PRESERVED / REGRESSION)
