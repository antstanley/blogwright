...
**Implements:** [2026-07-26-analytics_plugin.md §Analytics plugin → Its own service clients (Add)](../../../changes/2026-07-26-analytics_plugin.md) (`S3TablesClient` - REST-JSON: create/get/delete for table buckets, namespaces, and tables; it lives in `blogwright-analytics`, not in core)
**Depends on:** 31, 32
...
- [ ] The client lives in `packages/analytics/src/aws/` and supplies its own service descriptor through task 31's transport seam. Core's `SIGNING_NAMES` gains nothing and `AwsClients` gains no `s3tables` key - a test or review step asserts `grep -rn "s3tables" packages/core/src --include='*.ts' --exclude='*.test.ts'` returns nothing.
...
- [ ] Add a private `call<T>(method, path, payload?)` helper in the `packages/core/src/aws/microvms.ts:124-140` shape, sending the `s3tables` service descriptor with `content-type: application/json` and parsing an empty body as `{}`.
