...
- [ ] `analytics-catalog-integration` depends on `analytics-table` and creates the Glue `s3tablescatalog` federation that Firehose reads the table through, reaching AWS only through `clients.glue`.
